...
description: >
  ...
  "landing page", "设计稿还�?, "截图还原 HTML", "把图片复刻成网页", "微信小程序复�?, "小程序高保真", or when another skill needs image generation.
---
...
> **2. 你有现有�?App 截图或设计稿可以上传吗？**
> 有截�?�?可以保持导航/侧边栏的视觉一致性。没有也可以生成，但每次结果会有差异�?

> **3. 如果有截图，里面有没有你不希�?AI 改动的区域（比如侧边栏、顶部导航）�?*
> 这决定了用哪种参考图策略（见下方）�?
...
| 用户情况 | 策略 |
|---------|------|
| 没有截图，纯创意探索 | 不传参考图，完全自由生�?|
| 有截图，但只想锁定导�?侧边�?�?| 让用户提供或制作**纯净边框�?*（把内容区涂成纯色），只传边�?|
| 有截图，需要整体风格精准对�?| 传完整截图，但告知用户内容区创意会受影响 |

**纯净边框图怎么�?*：截一�?App 刚打开时内容区为空白的截图，或用任意工具把内容区覆盖为纯色�?
...
当用户想把生成图、截图或设计稿还原成 HTML/CSS，或直接还原成微信小程序 WXML/WXSS 时，先读�?`references/html-reconstruction.md`。核心原则：页面结构优先代码化；logo、品牌符号、复杂插画�?D/玻璃质感、半透明渐变等难复刻视觉元素要素材化。裁图只作为图生图参考和定位依据，最终放进页面的复杂资产要用图生图重绘，再裁边、抠图和清理边缘�?

微信小程序场景不要先生成 HTML 再机械转换。优先直接落�?WXML/WXSS �?TS/JS：布局、卡片、按钮、文本和常规图标用小程序代码实现；复杂插画、空状态、hero 装饰和品牌视觉单独生成为 PNG/WebP/SVG 资产，放�?`${miniprogramRoot}/assets/`。若 `miniprogramRoot` �?`miniprogram/`，实际目录就�?`miniprogram/assets/`，WXML 资源路径从该根目录写起，例如分别使用合法�?`mode="aspectFit"` �?`mode="widthFix"`。验收时在微信开发者工具固定设备预设、页�?viewport 宽高�?DPR；截图只取页面可视区，排除系统状态栏、胶囊按钮和工具栏，再用 `miniprogram-automator`（如需自动化）截图、pixel diff 与人�?side-by-side 检查�?

透明素材策略：厂�?logo、深�?wordmark、小号深色图标优先生成大尺寸纯白底素材，再用保守白底�?alpha；复杂彩色插画、hero 装饰、产品图优先绿幕或真实透明输出。不要把�?logo 和大插画塞进同一张素材板�?
...
**类比法（创意效果最好）**
�?这个工具像什�?，让 AI 借用那个参照物的设计语言，而不是描述布局�?
...
**清单法（最稳定，适合需要准确落地的页面�?*
列出页面上有哪些信息，不说怎么排，�?AI 自己决定布局�?

```
页面包含：用户名和头像、近30天数据趋势图、活�?Campaign 列表（名�?状�?触达数）、快捷操作入口�?
```

### 质量规则

- **不写排版规格**（像素、列数、padding）�?描述越具体，AI 越像在执行指令而不是做设计，结果反而更�?
- **给真实示例数�?*，不�?placeholder �?`"2.3M views, 180K saves"` �?`"显示播放�?` 效果好十�?
- **颜色�?HEX**，不�?HSL �?`#f9f5f0` �?`hsl(28 25% 97%)` 对模型更准确
- 如果传了参考图，在 prompt 开头明确说明哪些区域需要保持不�?
- **Prompt 控制�?800 字以�?* �?超长会导�?server disconnect
...
3. 内置工具�?ZenMux 质量接近时，选择内置工具，减少额�?provider 依赖�?
...
**多张屏幕**：必须串行执行（一张完成再开下一张），不能并行。偶�?disconnect 属正常，重试即可�?
...
| `--ref` | 额外参考图（可重复）| �?|
| `--name` | 输出文件�?| auto |
| `-o` | 自定义输出路�?| `~/.local/share/draw/outputs/YYYY-MM-DD/` |
